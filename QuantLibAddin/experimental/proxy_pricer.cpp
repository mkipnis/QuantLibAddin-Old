...
int main() {
	...
	try 
	{
		...
		std::ifstream json_request_file("pricers/midcurve_price_request.json");
		...
		py::module_ midcurve_pricer = py::module_::import("pricers.midcurve_pricer");
		py::object PriceMidCurves = midcurve_pricer.attr("PriceMidCurves");
		py::object midcurves = PriceMidCurves(midcurve_price_request);

		std::cout << "MidCurves : " << midcurves.cast<std::string>() << std::endl;
